...
function createSession(input: {
  ...
  return {
    id: `${input.event.id}-${input.scheduleRuleId ?? "rule"}-${input.date}-${time.replace(":", "")}-${input.suffix}`,
...
export async function loadCalendarData(): Promise<CalendarResponse> {
  ...
  try {
    if (!useSupabaseRpc) {
      throw new Error("RPC disabled; using schedule_rules directly");
    }
    ...
    // If the RPC signature differs during setup, schedule_rules still provide a working first pass.
  }
  ...
  return {
    ...
    integration: {
      ...
      notes: [
        "Loaded from Supabase event_series, schedule_rules and booking_types.",
        "DesignMyNight remains the secure payment/pre-order handoff.",
      ],
    },
  };
}
