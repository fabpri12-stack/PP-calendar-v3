...
function AppRouter() {
  return (
    <Switch>
      <Route path="/" component={CalendarPrototype} />
      <Route component={CalendarPrototype} />
    </Switch>
  );
}
      ...
      <section className="relative mx-auto flex w-full max-w-[1480px] flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="journey-hero">
          <div className="liquid-panel hero-copy compact-hero">
            <Badge className="w-max border-white/15 bg-white/12 text-white" data-testid="badge-concept">
              Booking engine v2
            </Badge>
            <div className="space-y-4">
              <h1 data-testid="text-hero-title">Calendar first. Payment last.</h1>
              <p data-testid="text-hero-copy">
                Date discovery, show preview, availability check, guest details, secure DesignMyNight payment transfer
                and a branded return/welcome page.
              </p>
            </div>
            <div className="hero-points" aria-label="Prototype highlights">
              <span><CalendarDays size={16} /> Date-led journey</span>
              <span><UserRound size={16} /> Customer details before payment</span>
              <span><ShieldCheck size={16} /> Secure DMN transfer</span>
            </div>
          </div>
        </section>
        ...
        <section className="engine-layout" aria-label="Purple Peacock booking widget">
          <section id="calendar" className="liquid-panel calendar-shell primary-calendar">
            <div className="panel-heading">
              <div>
                <p className="eyebrow">Start with the date</p>
  ...
  return (
    <header className="topbar liquid-panel">
      ...
      <nav aria-label="Prototype navigation">
        ...
        <button data-testid="button-scroll-journey" onClick={() => scrollTo("journey-panel")}>Booking flow</button>
      </nav>
    </header>
  );
}
      ...
      <div className="journey-content">
        ...
        {bookingStep === "event" && (
          <EventStep
            ...
            onContinue={() => setBookingStep("availability")}
          />
        )}
    ...
    <div className="step-tabs" aria-label="Booking progress">
      {steps.map((step, index) => (
        <button
          ...
          onClick={() => available && setBookingStep(step.id)}
          disabled={!available}
        >
  ...
  return (
    <div className="booking-step">
      <div className="date-change-row">
        <div>
          <p className="eyebrow">Change date or time</p>
          <h3>{formatDate(selectedDate)}</h3>
        </div>
        <Badge className="border-white/15 bg-white/10 text-white">Can be replaced with live DMN slots</Badge>
      </div>
...
function AvailabilityStep({
  ...
  const availabilityCopy =
    session.status === "limited"
      ? `${session.remaining} spaces showing. This would call DMN booking-availability before payment.`
      : session.status === "enquire"
        ? "This date can still continue as an enquiry-led booking."
        : "Availability check passed for the selected slot.";
    ...
    <div className="booking-step">
      <label className="form-field">
        ...
        <select
          ...
          onChange={(event) => setCustomer({ ...customer, guests: Number(event.target.value) })}
        >
      ...
      <div className="availability-result" data-testid="status-availability">
        <ShieldCheck size={20} />
        <div>
      ...
      <div className="button-pair">
        ...
        <Button data-testid="button-continue-details" className="book-button" onClick={onContinue}>Continue to details</Button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="booking-step transfer-screen" data-testid="screen-dmn-transfer">
      ...
      <div className="button-pair">
        ...
        <Button data-testid="button-preview-confirmation" variant="outline" className="glass-action" onClick={onComplete}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-modal border-white/15 bg-[#14091f]/88 text-white backdrop-blur-2xl sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-white/66">
            This can stay as an internal modal or be swapped for your website-builder popup trigger.
          </DialogDescription>
        </DialogHeader>
        ...
        {mode === "menu" && (
          <div className="modal-grid" data-testid="modal-menu">
            <div className="menu-card">
              <GlassWater />
              <strong>Menu button logic</strong>
              <p>
                In the final site, this can be an Elementor/Nova popup, a PDF or a CMS-controlled menu panel.
              </p>
            </div>
            <div className="menu-card">
              <Ticket />
            ...
            {event.videoUrl ? (
              ...
              <div className="video-placeholder">
                ...
                <div className="video-placeholder-copy">
                  ...
                  <strong>Show reel ready slot</strong>
                  <p>Add a YouTube, Vimeo or hosted show reel URL and this becomes the event preview before booking.</p>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <main className="loading-screen">
      <div className="liquid-panel loading-card" data-testid="status-error">
        ...
        <p>The prototype could not load event data. The designed error state keeps the journey polished.</p>
      </div>
    </main>
  );
}
